...
<html lang="<?= Yii::$app->language ?>" xmlns="http://www.w3.org/1999/html">
    <head>
        ...
        <script type="text/javascript">
            var editUrl = "<?=yii::$app->urlManager->createUrl('user/update')?>";
            var deleteUrl = "<?=yii::$app->urlManager->createUrl('user/delete-one')?>"
            var deleteallUrl = "<?=yii::$app->urlManager->createUrl('user/delete-more')?>"
            var detailUrl = "<?=yii::$app->urlManager->createUrl('user/find-one')?>"
                    ...
                    <?php foreach ($users as $index => $val){?>
                        <tr align="left">
                            ...
                            <td align="center">
                                ...
                                |&nbsp;&nbsp;--><?if($delete){?><a href="javascript:deleteUser('<?=$val->id?>')">删除</a><?}?><!--&nbsp;&nbsp;
...
<form action="<?=yii::$app->urlManager->createUrl('user/find-by-attri')?>" method="get" id="pageForm">
    <input type="hidden" id="page" name="page" value="<?=$pages->page?>"/>
    <input type="hidden"  name="r" value="user/find-by-attri"/>
