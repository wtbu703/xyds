...
<script type="text/javascript">
    var pageUrl = "<?=yii::$app->urlManager->createUrl('platform-recruit/find-by-attri')?>";
    var editUrl = "<?=yii::$app->urlManager->createUrl('platform-recruit/update')?>";
    var deleteUrl = "<?=yii::$app->urlManager->createUrl('platform-recruit/delete-one')?>"
    var deleteallUrl = "<?=yii::$app->urlManager->createUrl('platform-recruit/delete-more')?>"
    var detailUrl = "<?=yii::$app->urlManager->createUrl('platform-recruit/find-one')?>"
</script>
<script language="javascript" type="text/javascript" src="js/admin/platform-recruit/listall.js" charset="utf-8"></script>
</head>
<body>
    <div class="pad-lr-10">
        <div class="table-list">
            <table width="100%" cellspacing="0" id="user_list">
                ...
                <tr align="left">
                    <th width="80px"><input type="checkbox" id='check_box' onclick="selectall('id')"/>全选/取消</th><th width="30px">序号</th><th width="360px">职位</th><th width="160px">手机</th><th width="160px">邮箱</th><th width="160px">联系人</th><th align="center">操作</th>
                </tr>
                </thead>
                <tbody id="user_list_body">
                <?if(!is_null($platformRecruit)){?>
                <?php foreach ($platformRecruit as $index => $val){?>
                    <tr align="left">
                        <td><input type="checkbox" id="id" name="id" value="<?=$val->id?>"/></td>
                        <td><?=$index+$pages->page*$pages->pageSize+1?></td>
                        <td><a href="javascript:detail('<?=$val->id?>','<?=$val->position?>')"><?=$val->position?></a></td>
                        <td><?=$val->mobile?></td>
                        <td><?=$val->email?></td>
                        <td><?=$val->contacts?></td>
                        <td align="center">
                            <a href="javascript:openedit('<?=$val->id?>','<?=$val->position?>')">修改</a>&nbsp;&nbsp;
                            |&nbsp;&nbsp;<a href="javascript:deletePlatformRecruit('<?=$val->id?>')">删除</a>
                        </td>
                   </tr>
                    <?}?>
                    <?}?>
...
<form action="<?=yii::$app->urlManager->createUrl('platform-recruit/find-by-attri')?>" method="get" id="pageForm">
    <input type="hidden" id="page" name="page" value="<?=$pages->page?>"/>
    <input type="hidden"  name="r" value="platform-recruit/find-by-attri"/>
